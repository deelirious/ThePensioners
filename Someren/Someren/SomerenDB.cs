...
namespace Someren
{
    class SomerenDB
    {
        ...
        public static void DB_removeSupervisor(string remove_supervisor)
        {
            ...
            String sql = @"UPDATE Teachers SET [IsSupervisor] = '0' WHERE convert(varchar,LastName) = @LastName";

            //start connection with DB
            SqlCommand command = new SqlCommand(sql, connection);
            
                command.Parameters.AddWithValue("@LastName", remove_supervisor);
                command.ExecuteNonQuery();
        ...
        public static void DB_AddSupervisor(string add_supervisor)
        {
            ...
            String sql = @"UPDATE Teachers SET [IsSupervisor] = '1' WHERE convert(varchar, LastName) =  @LastName";

            //start connection with DB
            SqlCommand command = new SqlCommand(sql, connection);
            
                command.Parameters.AddWithValue("@LastName", add_supervisor);
                command.ExecuteNonQuery();
